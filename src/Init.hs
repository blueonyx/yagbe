...
module Init (initGBE, tiles, tilemap) where
...
initGBE = do
  ...
  liftIO $ do
    ...
    fillTileMap _stMemory tilemap

-- copy bytes into the memory starting at address $9000
fillTileBuffer :: (Ptr CUChar) -> [CUChar] -> IO ()
fillTileBuffer = fillMemory tileBufferAddress

-- copy bytes into the memory starting at address $9800
fillTileMap :: (Ptr CUChar) -> [CUChar] -> IO ()
fillTileMap = fillMemory tileMapAddress
