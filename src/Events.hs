...
registerKeyPress = do
  ...
  -- switch to rendering between tilebuffer and tilemap
  kp $ do
    "x" <- GTK.eventKeyName
    backToRio $ stRenderBuffer %= not
  ...
  kp $ do
    "t" <- GTK.eventKeyName
    backToRio $ stPaletteIndex %= (\x -> if x+1 == bgPaletteCount then 0 else x+1)

  -- prev bgPalette
  kp $ do
    "r" <- GTK.eventKeyName
    backToRio $ stPaletteIndex %= (\x -> if x == 0 then bgPaletteCount-1 else x-1)

  -- log keyPress
  kp $ do
    kv <- GTK.eventKeyName
    backToRio $ logInfo $ displayShow kv
